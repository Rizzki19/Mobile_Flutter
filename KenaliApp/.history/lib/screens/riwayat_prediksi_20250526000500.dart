...
class _RiwayatPrediksiState extends State<RiwayatPrediksi> {
  ...
  // Fungsi untuk menentukan apakah hasil prediksi berisiko
  bool _isRiskyPrediction(String? prediction) {
    return prediction != null && prediction.toLowerCase().contains('anda beresiko terkena stroke');
  }
  ...
  Widget _buildPredictionResult(String? prediction) {
    ...
    Color textColor = isRisky ? Colors.red : Colors.green;
    Color bgColor = isRisky ? const Color(0xFFFFEBEE) : const Color(0xFFE8F5E9);
    IconData icon = isRisky ? Icons.warning : Icons.check_circle;
      ...
      child: Row(
        children: [
          ...
          Column(
            ...
            children: [
              Text(currentTime, style: const TextStyle(color: Colors.black87, fontSize: 20)),
              Text(currentDate, style: const TextStyle(color: Colors.black54, fontSize: 12)),
            ],
          ),
        ],
      ),
