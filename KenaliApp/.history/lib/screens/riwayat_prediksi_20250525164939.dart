...
class _RiwayatPrediksiState extends State<RiwayatPrediksi> {
  ...
  void _showDetailDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: const Text('Detail Prediksi', style: TextStyle(fontWeight: FontWeight.bold)),
        content: SingleChildScrollView(
          child: ListBody(
            children: [
              Text('Jenis Kelamin: $genderValue'),
              Text('Usia: ${usiaController.text} tahun'),
              Text('BMI: ${bmiController.text}'),
              Text('Kadar Gula Darah: ${gulaDarahController.text}'),
              Text('Hipertensi: $hipertensiValue'),
              Text('Penyakit Jantung: $jantungValue'),
              Text('Status Menikah: $menikahValue'),
              Text('Pekerjaan: $pekerjaanValue'),
              Text('Area Tinggal: $areaValue'),
              Text('Perokok: $rokokValue'),
            ],
          ),
        ),
        actions: [
